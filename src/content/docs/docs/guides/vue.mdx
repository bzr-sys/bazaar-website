...
- (Optional) A [Netlify](https://www.netlify.com/) account for deploying your app, if desired.
...
Open up your terminal and make your current working directory is the one where you intend to create a project. Then, Run the following command in your command line:
...
With the project cleaned up, you are now ready to integrate Bazaar for backend services.
...
- `appId`: Checks for a `VITE_APP_ID` environment variable and if not present, use `test`, the Bazaar mock server default.
- `loginRedirectUri`: Checks for a `VITE_URL` environment variable and if not present, use `window.location.href`as the default, e.g. `http://localhost:5173/`.
- `onApiConnectError`: If a connection error occurs, log out the user.
...
First, add login and logout buttons to your template:
...
After you log in the the pop-up will close. Refresh the page and in your developer console you'll see:
...
Next, refactor the code so that the login status updates immediately after a successful login, not just on page load.
...
Now, log out and log back in and you'll see `is logged in: true` in your console after the pop-up closes, without the need for a page refresh.

Now, you have added user authentication to your app. In the next step, you will conditionally display content based on the user's login status.
...
In these changes, you create a reactive variable for the login status and set the value of that variable to `true` when a user is logged in.
...
Now, the app displays the appropriate button based on the user's login status.

Next, you'll add the functionality to create todos.

## Step 7 — Adding a Todo

In this step, you'll implement the feature to create a todo.
...
In this code, you define a `Todo` type, create a reactive variable for the new todo text and another for holding the todos.
...
Next, add the form to your template:
...
In these changes, you hook up the `onSubmit` handler to the form and bind the `todoText` reactive variable you just created to the input.

You have now implemented the functionality to add new todos. In the next step, you will display these todos in your app.
...
In your browser, add a few todos. Notice, todos are added to the bottom of your list. Usually you want the newest todos to appear first. To do that, you'll reverse the fetched todos.

```vue title="App.vue" del={4} ins={5,6} ins="(fetchedTodos.value = docs)"
<script setup lang="ts">
...
...
In these changes, you store fetched todos in a new reactive variable, `fetchedTodos`, and update `todos` to be a computed property that will updated when `fetchedTodos` changes. The spread operator (`...`) is used to create a new array, as the `reverse` method updates an array in place. Doing `fetchedTodos.value.reverse()` would cause an infinte re-render.
...
In this code, `todosCollection.subscribeAll` subscribes to all changes in the `todos` collection, and `arrayMirrorSubscribeListener` ensures that the local `todos` array stays in sync with the collection.

You can write your own subscribe listener. The syntax looks like this:
...
In `onSubmit`, you no longer need to add the todo to your local state as it will be added in the `arrayMirrorSubscribeListener` when an `onAdd` subscribe change event occurs.
...
This will render a delete button for each todo.

With that, your todo app is complete and you're ready to deploy.

## Step 12 — Deploying to Netlify

In this step, you will deploy your Vue.js Todo app to Netlify. You can deploy a Bazaar-powered app anywhere static sites can be hosted. Netlify provides an impressively frictionless deployment process and free static site hosting for public GitHub repos.
...
Next, go to Netlify, create an account, and click **Add new site**. Select **Import an existing project** and choose **GitHub**. Select your repository `vue-todo-guide`. If you can't find your repo, click **Configure the Netlify app on GitHub** to grant access.

Configure the site settings and build settings as follows:

- **Site name**: `vue-todo-guide`
...
To get your Bazaar app ID, open up [cloud.bzr.dev](https://cloud.bzr.dev/) and create a free account. Click on the user icon, top right, then click **Developers**. Next, click **Create App**.
...
Return to Netlify and paste in your app ID as the `VITE_APP_ID` value, and click **Deploy**. You'll see a notice, **Site deploy in progress**. The process will take a few minutes afterwhich your todo app will be deployed!

## Conclusion

In this tutorial, you built and deployed a Todo app using Vue.js and Bazaar. You set up a new Vue project, integrate Bazaar for backend services, implement user authentication, add and display todos, enable realtime syncing, and deploy your app to Netlify.
