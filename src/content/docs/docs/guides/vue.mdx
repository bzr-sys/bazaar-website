...
The `arrayMirrorSubscribeListener` implements `onIntial` so that data from a collection is fetched and added to your local state. Essentially, it's syntactic sugar for `todosCollection.getAll().then((docs) => (todos.value = docs));`.
...
Here is the completed `App.vue` file:
...
async function doLoginActions() {
  ...
  todosCollection.subscribeAll(
    {},
    arrayMirrorSubscribeListener(fetchedTodos.value),
  );
}
