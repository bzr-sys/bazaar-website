...
With the project cleaned up, you are now ready to integrate Bazaar for authentication and realtime data storage.
...
In this code, you define a `Todo` type, create two reactive variables, one for the new todo text and another for holding the list of todos.
...
<Aside title="Bazaar Databases">
  ...
  tables in a relational database. Collections are made up of documents, or
...
You have now implemented the functionality to add new todos, but they are not yet being displayed.

<Aside>
  To verify todos are being created in the database and added to local state,
  you can use the [Vue Devtools browser extension](https://devtools.vuejs.org/)
  and view the `todos` local state. You'll see todos added with the ID returned
  from the database.
</Aside>
...
In your browser, add a few more todos and notice they are added to the bottom of your list. Usually you want the newest todos to appear first. To do that, you'll reverse the fetched todos.
...
You can write your own subscribe listeners. The syntax looks like this:
