...
In your new project folder, create the folders `css` and `js`. Then, create the files `index.html`, `css/style.css`, `js/app.js`. You file structure will look like this:
...
3. Finally, add the following to `app.js`:
   ...
   The JavaScript is added within a function that's run when the `DOMContentLoaded` event occurs. This prevents global variable leaking and ensures the code runs only after the DOM has fully loaded, allowing safe manipulation of HTML elements.

</Steps>

Your project is set up. Next, you'll run a local HTTP server to view it in the browser.

## Step 3 — Running a local HTTP Server
...
## Step 4 — Running the Mock Server
...
## Step 5 — Initializing a Bazaar Client

In this step, you will install and initialize the Bazaar client to manage backend services for your Todo app.

You can use Bazaar directly from a CDN via a script tag. Add the script tag before the `app.js` tag:
...
Here we are using unpkg, but you can also use any CDN that serves NPM packages, for example [jsdelivr](https://www.jsdelivr.com/) or [cdnjs](https://cdnjs.com/).
...
```html title="index.html" del={4} ins={5,6}
<div class="container">
  <h1>Todo</h1>
  <!-- Todo markup will be added here -->
...
In the error above, the mock server accepts requests from `http://localhost:8080` and is missing a trailing slash as HTTP server is serving on `http://localhost:8080/`.
...
## Step 7 — Displaying Logged-in Content
...
In those changes, you add a `logged-out` class to the `body` tag, and a `main` tag, where logged in content will be displayed.
...
Next, add update the `doLoginActions` function to conditionally display content in `app.js`:
...
Now, on log in, the `logged-out` class is removed from the `body` element, displaying the log out button and the `main` element. And the log in button is removed from the DOM as it's not needed once a user has logged in.
...
These changes add a form with a single input field for the todo text. Notice, the form has no `action`, you'll add a submit event listener in JavaScript next.
...
Next, update the `doLoginActions` function to fetch todos:

```js title="app.js" ins={"Create a JSDoc type definition for a todo":6-13} ins={15-32} ins={34-43} ins={"Fetch todos from the database and insert into the DOM one by one":45-52}
async function doLoginActions() {
  ...
...
## Step 10 — Adding Realtime Syncing
...
In this code, `todosCollection.subscribeAll` subscribes to all changes in the `todos` collection. You pass a subscribe listener as the second `subscribeAll` argument with functions to handle `onInitial` and `onAdd` events. In both cases, you insert a todo into the DOM, exactly as you did previous in `todosCollection.getAll`.

You can think of `onInitial` as being syntactic sugar for `todosCollection.getAll`. It fetches all docs from the collection, then calls your `onInitial` function for each doc.

With the `onAdd` handler in place, there's no need to also add a todo to the DOM when the form is submitted. Update the submit event listener:

```js title="app.js" del={4}
todoForm.addEventListener("submit", (event) => {
  ...
...
Realtime syncing enabled, but only handling add todo events. In the next step, you will implement the functionality to toggle the completion status of todos and update your subscribe listener with a `onChange` handler.

## Step 11 — Toggling Todo Completion

In this step, you'll implement functionality to toggle the completion status of todos.

First, update todo template:

```js title="app.js" ins={6-8,10,15-24}
function insertTodoInDOM(todo) {
    ...
    todosElement.insertAdjacentHTML(
      "afterbegin",
...
Then, you add a button and event listener for toggling todo completion. To determine the if a todo is completed or not, you check for the presence of the `completed` class. If it is present, set the todo `completed` value to `false`, otherwise, set it to `true`. 
...
The `onChange` forces the presence of the `completed` class to the `completed` value of the changed doc by passing the value as the second argument of `todoElement.classList.toggle`.

You can now mark todos as completed or incomplete. In the next step, you will add the functionality to delete todos.

## Step 12 — Deleting a Todo

In this step, you will implement the functionality to delete todos.

First, update the todo template:
...
The delete handler removes the todo with the corresponding ID from the DOM. The [optional chaining operator (?)](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Optional_chaining) checks for the presence of the element before deleting.
...
Next, go to Netlify, create an account if necessary, and click **Add new site**. Select **Import an existing project** and choose **GitHub**. Select your repository `vanilla-js-todo-guide`. If you can't find your repo, click **Configure the Netlify app on GitHub** to grant access.
...
Leave the other fields blank. Note down your Netlify URL, found right under the **Site name** field. Then, click **Deploy**.

The process will take a few minutes, after which your todo app will be deployed, but the Bazaar config still needs to be updated. We deploy first, before updating the config because the app URL is required.

Next, you need a Bazaar app ID, open [cloud.bzr.dev](https://cloud.bzr.dev/) and create a free account. Click on the user icon at the top right, then click **Developers**. Next, click **Create App**. Give your app a name. I'm using **Vanilla JS Todo Guide**. Set your **APP URL**, which should be the same as your app's Netlify URL, e.g. `https://vanilla-js-todo-guide.netlify.app`. Finally, click **Create**, then copy your app ID. It will look something like `01907291-d4e5-79d5-a312-ba20260c9b98`.

Now you can update your Bazaar client config. As you're not using a build step, you can't make use of environment variables so you'll hardcode production values:
...
Notice `bazaarUri` is removed so it defaults to its production value.

Finally, commit your changes and push to deploy:
...
Pushing will trigger Netlify to deploy your site. In a few minutes your app will be deployed!
