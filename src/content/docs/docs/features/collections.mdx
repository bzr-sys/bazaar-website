...
Bazaar provides document-orientated (NoSQL) and schema-less databases scoped to a user and app. A database contains collections which are analogous to tables in a relational databases. Collections are made up of documents, or docs, analogous to rows in a relational databases.
...
The listener is an object that contains optional functions to be run when an object is added, updated, or deleted. It has type
