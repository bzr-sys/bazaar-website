...
RethinkID realtime databases are document-orientated (NoSQL) and schema-less.

## Database Architecture

Databases are created per user and per app. Each user gets their own database for your app. And for any other RethinkID-powered app used, a user gets their own separate database.
...
Create collections manually using the `rid.collections.create` method.
...
```js
const exampleCollection = rid.collection("example-collection", options);
```

The `options` parameter is optional.

When using TypeScript, you can specify the type of the collection doc. Docs must have an `id`.
...
const tasksCollection = rid.collection<TaskDoc>("tasks");
```
...
<Tabs>
  ...
  <TabItem label="TS">
    ```ts
    import { OrderByType, FilterObject, CollectionGetAllOptions } from "@rethinkid/rethinkid-js-sdk";

    const filter: FilterObject = {}; // No filter
    const options: CollectionGetAllOptions = { startOffset: 10, endOffset: 20, orderBy: { height: OrderByType.DESC } };
...
Like `getAll`, `getPage` also takes an optional [`filter`](#filtering).
